...
const  eltMyBooks = document.getElementById("myBooks");// méth js de Doc retournant 1 objet Elt du DOM associé à l'ID myBooks
...
btnToSearchBook();// méth d'ajout btn ajout livre
ChargementPochList();// méth chargem livres dans pochlist
...
function btnToSearchBook(){
	...
	eltForms.innerHTML = '<input type="button" id="ajoutLivre" value="Ajouter un livre">';//l'attribut innerHtml de l'ojet eltForms renvoie le button...
	console.log(eltMyBooks);
	buttonAddLivre=document.getElementById("ajoutLivre");
	buttonAddLivre.addEventListener('click',function(event){ //méthode à 3 param:type, listener et opt
...
function clickAjoutLivre(){
	preventDefault();//neutralise l'action par défaut
	EcranSaisie();
	console.log(eltMyBooks);
}
...
function EcranSaisie(){
	...
	eltButtonAnnuler.addEventListener('click',function(event){
		event.preventDefault();//annulat action en cours
		btnToSearchBook();//affichage bouton 'Ajouter un livre' et du champ 'Nouveau livre'
    	...
    	if (this.readyState == XMLHttpRequest.DONE && this.status == 200) {
			...
			if (response.totalItems != "0") {
				...
				for (let livre of response.items) {   
					...
					newDiv.classList.add('bookItem');//ajoute à newDiv la classe 'booItem'
					...
					newDiv.append(newDescription);//insère 'newDescription' à 'newDiv'
					...
					let imageLivre = "unavailable.png";
